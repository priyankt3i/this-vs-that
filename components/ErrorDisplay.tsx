...
const WittyIcon: React.FC = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-6 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" />
    </svg>
);

const ErrorIcon: React.FC = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-6 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const ErrorDisplay: React.FC<ErrorDisplayProps> = ({ message, isWitty = false }) => {
    ...
    const errorClasses = "bg-red-900/30 border-red-500/50 text-red-300";
    const wittyClasses = "bg-amber-900/30 border-amber-500/50 text-amber-200";
    
    const titleClasses = "text-2xl md:text-3xl font-bold block";
    const subtextClasses = "block mt-3 text-lg text-slate-300 max-w-prose";
